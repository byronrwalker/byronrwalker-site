(function ($) {
    ...
    function initialiseGoogleMap() {
        ...
        if (map.data("zoom")) map_zoom = map.data("zoom");
    ...
    function sendMessage(e) {                  
        ...
        oForm.form = $('#messageForm');
        oForm.messageFeedbackContainer = $('messageFeedbackContainer');
        oForm.messageFeedback = $('#messageFeedback');
        var rsFormErrors = false;
        oForm.errors = false;
        var rsFormAction = rsForm.attr('action');
        oForm.action = oForm.form.attr('action')
        var rsFormCaptcha = rsForm.data('captcha');
        oForm.captcha = oForm.form.data('captcha');
        var rsFormFields = rsForm.find('.input-field');		           	
        oForm.fields = oForm.form.find('.input-field');
        var rsFormName = rsForm.find("[name='rsName']");
        oForm.name = oForm.form.find("[name='rsName']");
        var rsFormEmail = rsForm.find("[name='rsEmail']");
        oForm.email = oForm.form.find("[name='rsEmail']");
        oForm.subject = oForm.form.find("[name='rsSubject']");
        var rsFormMessage = rsForm.find("[name='rsMessage']");		
        oForm.message = oForm.form.find("[name='rsMessage']");
        var rsFormResponce = rsForm.find('.rsFormResponce');
        oForm.response = oForm.form.find('.rsFormResponce');
        
        
        console.log(oForm);

        // Button ripple effect
        ripple(oForm.form.parent(), e.pageX, e.pageY);

        // Reset form errors
        oForm.fields.removeClass('error');
        oForm.errors = false;
        //rsFormErrors = false;						
        ...
        if(!oForm.name.val()) {
            oForm.errors = true;
            oForm.name.parent().addClass('error');
        }

        if(!oForm.email.val() || !isValidEmail(oForm.email.val())) {
            oForm.errors = true;
            oForm.email.parent().addClass('error');
        }
        
        if(!oForm.subject.val()) {
            oForm.errors = true;
            oForm.subject.parent().addClass('error');
        }

        if(!oForm.message.val()) {
            oForm.errors = true;
            oForm.message.parent().addClass('error');
        }
        ...
        if(oForm.errors) {
            ...
            if(IsRecapchaValid()){
                console.log("Is Valid Captcha");
                $.ajax({
                    url: "https://formspree.io/byron@kyrotechnologies.com", 
                    method: "POST",
                    data: {name:oForm.name.val(),email:oForm.email.val(),subject:oForm.subject.val(),message: oForm.message.val()},
                    dataType: "json",
                    ...
                    error: function (textStatus, errorThrown) {
                        alert("There was an issue sending you message, please try again later.");
                    }
                });
            }else{
                alert("There was a recaptcha issue, we cannot send your message at this time");
            }
        }					                         
    }
...
var onloadCaptchaCallback = function (){
    widId = grecaptcha.render('recapchaWidget', {'sitekey':'6LeAlycTAAAAAERJbtqH6pZ9MeruuxCbz4XFsWf7'});
};
function IsRecapchaValid(){
    console.log("IsRecapchaValid?");
